...
function PageGlobalChat (props) {
  ...
  async function getImgSrc () {
    const image_result = await sendFile(file[0]).then(value => value.imgSrc)
    return image_result
  }

  async function getAudioSrc () {
    const audio_result = await sendAudio(audio).then(value => value.audioSrc)
    return audio_result
  }
